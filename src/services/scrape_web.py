...
def get_scrape_web_data(TropicalCyclone, ForecastTrack, TrackHistory, db):
    ...
    description_text_list = [
        element.text for element in soup.find_all("a", {"href": True})]
    ...
    for i, images in enumerate(images_list):
        if i > 2:
            image_src = 'https://rammb-data.cira.colostate.edu/' + \
                images['src']
    ...
    if places_list and formatted_description_text_list and description_id_list and formatted_images_list:
        # combined_list
        combined_list = [list(e) for e in zip(
            places_list, formatted_description_text_list, description_id_list, formatted_images_list)]
        for item in combined_list:
            ...
            # add record to db
            existing_tropical_cyclone = TropicalCyclone.query.filter_by(
                description_id=description_id).first()
    ...
    if description_id_list:
        for description_id in description_id_list:
            URL = 'https://rammb-data.cira.colostate.edu/tc_realtime/storm.asp?storm_identifier={}'.format(
                description_id)
            page = requests.get(URL)
